...
University in early 2009.\footnote{{\tt
...
associated with each term that appears in the collection.\footnote{In
  information retrieval parlance, {\it term} is preferred over {\it
...
id and a {\it payload}---information about occurrences of the term in
...
payload, however, is term frequency ({\it tf}), or the number of times
...
\caption{Simple illustration of an inverted index.  Each term is
  ...
  document id and a payload, denoted by {\it p} in this case.  An
...
we see that {\it term}$_1$ occurs in $\{d_1, d_5, d_6, d_{11},
\ldots\}$, {\it term}$_2$ occurs in $\{d_{11}, d_{23}, d_{59}, d_{84},
\ldots\}$, and {\it term}$_3$ occurs in $\{d_1, d_4, d_{11}, d_{19},
\ldots\}$.  In an actual implementation, we assume that documents can
be identified by a unique integer ranging from 1 to {\it n}, where
{\it n} is the total number of documents.\footnote{It is preferable to
...
stored in structures called {\it accumulators}.  At the end (i.e.,
once all postings have been processed), the top {\it k} documents are
...
\caption{Simple illustration of the baseline inverted indexing
  ...
  Postings are shown as pairs of boxes ({\it docid}, {\it tf}).}
...
The first trick is to encode {\it differences} between document ids as
opposed to the document ids themselves.  Since the postings are sorted
by document ids, the differences (called {\it d}-gaps) must be
positive integers greater than zero.  The above postings list,
represented with {\it d}-gaps, would be:
...
be easily reconstructed from the {\it d}-gaps.  However, it's not
obvious that we've reduced the space requirements either, since the
largest possible {\it d}-gap is one less than the number of documents
in the collection.

This is where the second trick comes in, which is to represent the
{\it d}-gaps in a way such that it takes less space for smaller
...
Compression, in general, can be characterized as either {\it lossless}
or {\it lossy}:\ it's fairly obvious that loseless compression is
...
bandwidth), we can achieve the best of both worlds:\ smaller {\it and}
faster.
...
discussion to {\it unsigned} integers, since {\it d}-gaps are always
positive (and greater than zero).  This means that 1 and 4,294,967,295
both occupy four bytes.  Obviously, encoding {\it d}-gaps this way
...
order bit of every byte as the {\it continuation bit}, which is set to
...
of {\it d}-gaps), which slows down processing.

A variant of the varInt scheme was described by Jeff Dean in a keynote
talk at the WSDM 2009 conference.\footnote{\tt
...
32-bit word are reserved as a {\it selector}.  The remaining 28 bits
...
{\it must} consume multiples of eight bits, even when fewer bits might
...
{\it d}-gaps and term frequencies should never be zero.\footnote{As a
  ...
  classic IR text {\it Managing Gigabytes}~\cite{Witten_etal_1999}.}
...
Researchers have shown that Golomb compression works well for {\it
...
the number of documents in the collection.\footnote{For details as to
  ...
  distribution of {\it d}-gaps.  From this we can derive an optimal
  setting of $b$.}

Putting everything together, one popular approach for postings
compression is to represent {\it d}-gaps with Golomb codes and term
...
which, among other factors, determine the distribution of {\it
...
obtained (measured in bits per {\it docid}), Golomb and Rice codes
...
fastest, followed by varInt.\footnote{However, this study found less
  ...
  analysis, presumably due to the different distribution of {\it d}-gaps
...
indexing algorithm if we were to adopt Golomb compression for {\it
...
parameterless.  Compressing {\it d}-gaps with Golomb codes, however,
...
postings list (i.e., the document frequency, or {\it df}).  The first
is easy to obtain and can be passed into the reducer as a constant.
The {\it df} of a term, however, is not known until all the postings
...
term's {\it df} before any of its postings arrive.  This can be solved
...
$\langle t, \ast \rangle$ are emitted with the partial {\it df} as the
value.
...
\rangle$ key, associated with a list of values representing partial
       {\it df} values originating from each mapper.  Summing all
       these partial contributions will yield the term's {\it df},
...
partitioning strategies for distributed retrieval:\ {\it document
  partitioning} and {\it term partitioning}.  Under document
...
discriminating when it comes to {\it which} relevant documents appear
in their results (out of the set of {\it all} relevant documents).
...
Golomb codes for compressing {\it d}-gaps and $\gamma$ codes for term
...
Google search architecture.\footnote{\tt
...
textbook {\it Managing Gigabytes}~\cite{Witten_etal_1999} remains an
